package com.zebra.android.jb.barcode;
...
public class BarcodeManager {
	...
	//caleb.liao add start
	private File ht368dUartOe  = new File("/proc/jbcommon/gpio_control/scan_ir_gps_rs232_switch_oe");
	private File ht368dUartCs0  = new File("/proc/jbcommon/gpio_control/scan_ir_gps_rs232_cs0");
	private File ht368dUartCs1  = new File("/proc/jbcommon/gpio_control/scan_ir_gps_rs232_cs1");
	//caleb.liao add end
	...
	private byte[] N4313_timeout_6000 ={0x41,0x4F,0x53,0x4D,0x4F,0x54,0x03,0x00,0x00,0x00};//AOSMOT6000
	private byte[] N4313_continue ={0x50,0x41,0x50,0x50,0x4D,0x01};//PAPPM1
	...
	public void Barcode_Open(Context context, Callback dataReceive) {
		...
		if (device_type == 3) {
			...
			trigFile = new File("/proc/jbcommon/gpio_control/scan_switch");// 写"1"出光，写“0”关闭
			...
			//add by caleb.liao for ht368d
			writeFile(ht368dUartOe, "1");
			writeFile(ht368dUartCs0, "1");
			writeFile(ht368dUartCs1, "0");
			//add by caleb.liao for ht368d
			trig_scan = "0";
		} else {
	...
	public synchronized void Barcode_Stop() {
		Log.e(TAG, "Barcode_Stop() scan: "+isScan());
		if (isScan()) {
			if (hardwareVersion <= 103) {
				if (getScannerModel() == MODEL_EM3070) {
					// 3070trig 指令反过来
					...
					try {
						...
						if (size > 0) {
							if (onDataListener != null) {
								...
								Log.d(TAG,"ReadThread buffer: " + Tools.bytesToHexString(buffer)+" isCutData: "+isCutData);
								if (!isCutData) {
								...
								if(getScannerModel()==2){
									mHandler.postAtTime(sendBuffer,SystemClock.uptimeMillis() + 19);						
								}else {
...
//			writeFile(openCom, status);  //caleb.liao mofity
		writeFile(openCom, "0");
	}
	   ...
	   Log.d(TAG, "init() type: "+type);
		try {
			...
			case Protocol_N4313_Codeid:
				...
				setIsReceive(true);
				notifyReader();
                writeCommand(N4313_timeout_6000);
				mHandler.removeCallbacks(checkInitSuccess);
	...
	private void dealData(byte[] buffer, int size) {
		try {
			...
			if (buffer == null) {
				Log.v(TAG,"dealData buffer == null");
				Barcode_Stop();
				return;
			}
			Log.e(TAG, "dealData buffer size:" + size + " buffer lenght:" + buffer.length+" getScannerModel(): "+getScannerModel()+" str: "+str);
		...
		for (int i = 0; i < list_byte.size(); i++) {
			if (list_byte.get(i)[0] == (byte) 0x11
				...
				if (getScannerModel() == MODEL_EM3070) {
					...
					System.arraycopy(list_byte.get(i), 2, buffer, 0, buffer.length);
					if (null != onDataListener) {
						onDataListener.Barcode_Read(buffer, "", 0);
						notifyScanReader();
					}
